...
export default async function NewDocPage({
  ...
  if (spaces.length === 0) {
    return (
      <PageContainer>
        <EmptyState
          icon={<FolderLock />}
          title="No editable spaces"
          body="You have the editor role, but no space currently grants you edit access. Ask an admin to add you (or one of your groups) to a space's editors."
  ...
  return (
    <div>
      {chosen && (
        <div className="flex flex-wrap items-center gap-x-3 gap-y-1 border-b border-slate-200 bg-slate-50 px-6 py-2 text-sm text-slate-600 sm:px-8">
          ...
          <Link href={`/doc/new?template=pick${spaceQS}`} className="text-compass-700 hover:underline">
            Choose another
          </Link>
          <Link href={`/doc/new?template=blank${spaceQS}`} className="text-compass-700 hover:underline">
            Start blank
          </Link>
        </div>
      )}
